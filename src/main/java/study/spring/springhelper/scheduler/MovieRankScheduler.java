...
public class MovieRankScheduler {
	...
	public void collectMovieRank() {
		...
		String date = String.format("%04d-%02d-%02d", c.get(Calendar.YEAR), 
				c.get(Calendar.MONTH)+1, c.get(Calendar.DAY_OF_MONTH));
		
		/** 3) 영화 진흥원에 API 요청 */
		//영화진흥원 API에 전달한 날짜 형식은 yyyymmdd 이므로 준비된 날짜 문자열에서 "-"를 제거한다.
		...
		// 변수명이 너무 길어서 참조 변수 만듬
		List<DailyBoxOfficeList> list = null;
		
		// 검색결과가 있다면  list만 추출하고 조회결과가 없다면 처리 종료
		...
		// OpenAPI 조회결과의 Beans에 검색 날자 추가 후 로그를 통해 내역 확인
