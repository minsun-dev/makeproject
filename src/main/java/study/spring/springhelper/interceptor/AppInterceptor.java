...
public class AppInterceptor extends HandlerInterceptorAdapter {
	 ...
	 * 보통 이곳에서 각종 체크작업과 로그를 기록하는 작업을 진행한다. (제일 자주 사용됨)
	 * */
	@Override
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler)
		...
		// 웹 브라우저 정보
		// -> import uap_clj.java.api.Brower;
		...
		String browserStr = String.format("- Browser: {family=%s, patch=%s, majer=%s, minor=%s}",  
				browser.get("family"), browser.get("patch"), browser.get("major"), browser.get("minor"));
		
		String osStr = String.format("- OS: {family=%s, patch=%s, majer=%s, minor=%s}",  
				os.get("family"), os.get("patch"), os.get("major"), os.get("minor"));
	 ...
	 * 컨트로럴 로직이 실행된 이후 호출된다.
	 * 컨트롤러 단에서 에러 발생 시 해당 메서드는 수행되지 않는다.
	 * request로 넘어온 데이터 가공 시 많이 사용한다.
	 * */
	@Override
	...
	/**
	 * 컨트롤러 종료 후 view가 정상적으로 렌더링 된 후 제일 마지막에 실행이 되는 메서드(자주 사용안됨)
	 * */
	@Override
	...
	/**
	 * Servlet 3.0 부터 비동기 요청이 가능해 짐에 따라 비동기 요청 시 
	 * PostHandle와 afterCompletion메서드를 수행하지 않고 이 메서드를 수행하게된다.
	 * 거의 사용 안함
	 * */
	@Override
