...
<body>
			...
			<c:otherwise>
				...
				<c:forEach var="item" items="${output}" varStatus="status">
					...
					<c:if test="${keyword != '' }">
						...
						<%-- 출력을 위해 준비한 학과이름과 위치에서 검색어와 일치하는 단어를 형광펜 효과로 변경 --%>
						<c:set var="dname" value="${fn:replace(dname, keyword, mark)}"/>
						<c:set var="loc" value="${fn:replace(loc, keyword, makr)}"/>
					</c:if>
	...
	<%-- 페이지 번호 (시작 페이지부터 끝 페이지까지 반복) 오답: var="1"로 오타내서 페이지 번호 출력이 안됐음 --%>
